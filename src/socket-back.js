...
io.on("connection", (socket) => {
    ...
    socket.on("excluir_documento", async (nomeDoDocumento) => {
        const resultado = await excluirDocumento(nomeDoDocumento);

        if (resultado.deletedCount) {
            io.emit("excluir_documento_alert", nomeDoDocumento);
        }
    })
